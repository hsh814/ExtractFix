...
sed -i 's/xmlRealloc/realloc/g' ../buf.c
...
CC=$compiler CFLAGS="$cflags" ../autogen.sh --enable-static --without-threads --without-thread-alloc --with-threads=no --with-thread-alloc=no &> /dev/null
